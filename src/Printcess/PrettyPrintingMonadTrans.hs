...
pretty :: forall a m. (Monad m, Pretty a m) => Config → a → m String
pretty c
  = fmap ( concat
         . (`sepByL` "\n")
         . reverse
  ...
  . flip execStateT (PrettySt (_configInitIndent c)
  ...
  . runPrettyT
  . pp
  -- . ((addIndent +>) :: forall b. Pretty b m => b → PrettyT m () ) -- Add indentation to the first line.
                   -- The other lines are indented when they are reached.

fx :: (Monad m, Pretty a m) => a → PrettyT m ()
fx x = addIndent +> x

-- pp :: Pretty a m => a → PrettyT m ()
-- addIndent :: Monad m => PrettyT m ()
-- (+>) :: (Pretty a m, Pretty b m) => a → b → PrettyT m ()

prettyId :: Pretty a Identity => Config → a → String
prettyId c = runIdentity . pretty c
...
prettyPrint :: (MonadIO m, Pretty a m) => Config → a → m ()
prettyPrint c x = liftIO . putStrLn =<< pretty c x

-- Type Classes ----------------------------------------------------------------

type MonadPretty = MonadState PrettySt
...
class Monad m => Pretty a m where
...
instance Pretty Int    m where pp = pp . show
instance Pretty Float  m where pp = pp . show
instance Pretty Double m where pp = pp . show
instance (Pretty k m, Pretty v m) => Pretty (M.Map k v) m where
...
class Pretty1 f where
  pp1 :: Pretty a m => f a → PrettyT m ()
  default pp1 :: Pretty (f a) m => f a -> m ()
  pp1 = pp

-- | The 'Pretty2' type class lifts pretty printing to binary type constructors.
--   It can be used in special cases to abstract over type constructors which
--   are pretty printable for any pretty printable type argument.
class Pretty2 (f :: * → * → *) (m :: * → *) where
  pp2 :: (Pretty a m, Pretty b m) => f a b → PrettyT m ()
  default pp2 :: Pretty (f a b) m => f a b -> m ()
  pp2 = pp
...
newtype PrettyT m a = PrettyT { runPrettyT :: StateT PrettySt m a }
  deriving (Functor, Applicative, Monad, MonadState PrettySt)
type PrettyM = PrettyT Identity

instance Monad m => Pretty (PrettyT m ()) m where pp = (>> return ())
...
indented :: Pretty a m => a → PrettyT m ()
indented a = indent +> a +> unindent
...
withPrecedence :: (Assoc, Int) → PrettyT m () → PrettyT m ()
...
instance Pretty a m => Pretty (AssocAnn a) m where
  pp = \case
    L a → left a
    R a → right a
    I a → inner a
...
assocDir :: Pretty a m => Assoc → a → PrettyT m ()
assocDir a ma = do
  a' ← use assoc
  if | a' == a → pp ma
     | otherwise → do
...
sepBy :: (Pretty a m, Pretty b m) => [b] → a → PrettyT m ()
...
interleaveL :: (Pretty a m, Pretty b m) => a → [b] → PrettyT m ()
...
interleaveR :: (Pretty a m, Pretty b m) => a → [b] → PrettyT m ()
...
ifPrint :: Pretty a m => Bool → a → PrettyT m ()
ifPrint True  a = pp a
ifPrint False _ = pp ""
...
ppListMap :: (Pretty a m, Pretty b m) => [(a, b)] → PrettyT m ()
ppListMap = block . map (\(a,b) → a ++> "→" ++> b)

-- | Print a @Data.Map@ in the same way as @ppListMap@.
ppMap :: (Pretty a m, Pretty b m) => M.Map a b → PrettyT m ()
...
--
--   > ppParen x = "(" +> x +> ")"
ppParen ∷ Pretty a m ⇒ a → PrettyT m ()
ppParen x = "(" +> x +> ")"
...
--   > -- ↪ "----- Foo -------------------------------…"
ppBar ∷ Pretty a m => Char → a → PrettyT m ()
ppBar c s = do
  w ← use maxLineWidth
...
nl :: Monad m => PrettyT m ()
nl = do
  text %= ("" NE.<|)
  addIndent

-- | Print a space.
sp :: Monad m => PrettyT m ()
sp = pp " "
